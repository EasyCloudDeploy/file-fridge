...
def upgrade() -> None:
    ...
    op.execute("UPDATE remote_connections SET trust_status = UPPER(trust_status) WHERE trust_status IS NOT NULL")
    ...
    with op.batch_alter_table('remote_connections', schema=None) as batch_op:
        batch_op.alter_column('trust_status',
...
def downgrade() -> None:
    ...
    op.execute("UPDATE remote_connections SET trust_status = LOWER(trust_status) WHERE trust_status IS NOT NULL")
