...
def upgrade() -> None:
    ...
    op.add_column('cold_storage_locations',
    ...
    event_mapping = {
        ...
        'info': [
            'SCAN_COMPLETED', 'SCAN_ERROR', 'PATH_CREATED', 'PATH_UPDATED',
            'PATH_DELETED', 'DISK_SPACE_CAUTION', 'DISK_SPACE_CRITICAL'
        ],
    }

    # Migrate each notifier
    for notifier_id, filter_level in notifiers:
        events = event_mapping.get(filter_level.lower() if filter_level else 'info', [])
        conn.execute(
