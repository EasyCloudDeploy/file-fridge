...
def _serialize_file(
    ...
    file_dict = {
        ...
        "is_shareable": file.is_shareable is not False,
        "is_remote": False,
        "remote_peer_id": None,
...
def _serialize_remote_cached_file(
    ...
    *,
    ...
    return {
        ...
        "display_file_path": remote_file.display_file_path,
        ...
        "file_extension": remote_file.file_extension,
        "mime_type": remote_file.mime_type,
        "status": "active",
        "last_seen": (
            remote_file.last_announced_at.isoformat() if remote_file.last_announced_at else now_iso
        ),
        ...
        try:
            ...
            if filter_criteria.remote_only:
                ...
                if cursor:
                    try:
                        ...
                        query = _apply_cursor_pagination(
                            query, sort_field, cursor_data, is_descending
                        )
                ...
                next_cursor = (
                    _generate_next_cursor(files_list, sort_by, valid_sort_fields)
                    if has_more
                    else None
                )
            ...
            if filter_criteria.local_only:
                ...
                if filter_criteria.search:
                    ...
                    for term in search_params.get("positive_terms", []):
                        ...
                        remote_query = remote_query.filter(
                            or_(
                                RemoteSharedFileCache.file_path.ilike(
                                    f"%{escaped_term}%", escape="\\"
                                ),
                                RemoteSharedFileCache.display_file_path.ilike(
                                    f"%{escaped_term}%", escape="\\"
                                ),
                            )
                        )
                    ...
                    if search_params.get("size_val") is not None:
                        ...
                        if op == ">":
                            remote_query = remote_query.filter(
                                RemoteSharedFileCache.file_size > val
                            )
                        elif op == "<":
                            remote_query = remote_query.filter(
                                RemoteSharedFileCache.file_size < val
                ...
                aggregated_remote_rows: list[
                    tuple[RemoteSharedFileCache, P2PPeer, list[str], int]
                ] = []
            ...
            yield (
                json.dumps(
                    {
                        "type": "metadata",
                        "total": local_total_count + remote_total_count,
                        "page_size": page_size,
                        "has_more": has_more,
            ...
            for remote_file, peer, peer_names, peer_count in visible_remote_rows:
                try:
                    ...
                    logger.warning(
                        "Error serializing remote cached file %s from peer %s: %s",
                        remote_file.id,
                        peer.id,
                        e,
                    )
...
@router.post("/{inventory_id}/share", status_code=status.HTTP_200_OK)
...
def bulk_set_file_shareability(
    ...
    for file_id in request.file_ids:
        ...
        file.is_shareable = request.is_shareable
        successful += 1
