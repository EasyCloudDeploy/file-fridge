...
## 2026-03-24 - [MEDIUM] Fix Command-Line Option Injection in xattr
**Vulnerability:** The `app/services/criteria_matcher.py` file invoked the `xattr` subprocess with an unsanitized file path (`str(file_path)`). An attacker who manages to create a file with a name starting with a hyphen (e.g., `-d`) could have caused the `xattr` command to interpret the file path as an option, potentially leading to errors or unintended command execution depending on the utility.
**Learning:** When using `subprocess.run` to invoke command-line utilities with user-controlled or variable file paths, these paths might be parsed as options rather than positional arguments if they start with a hyphen.
**Prevention:** Always use the `--` token before the file path arguments in subprocess invocations (e.g., `["xattr", "-p", "com.apple.lastuseddate#PS", "--", str(file_path)]`). The `--` tells the utility that subsequent arguments are not options, even if they begin with a hyphen.
