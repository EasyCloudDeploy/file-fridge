...
## 2026-05-08 - [MEDIUM] Fix Command-Line Option Injection in System Calls
**Vulnerability:** When running system commands (like `diskutil` or `xattr`) via `subprocess.run`, file paths were passed without the POSIX double-dash (`--`) separator or without forcing absolute paths. If a file was named with a leading hyphen (e.g., `-foo.txt`), it could be mistakenly interpreted as a command-line flag by the target utility.
**Learning:** Utilities that parse command-line arguments can act unpredictably or insecurely if they mistake variable data (like filenames) for options. Since `diskutil` doesn't strictly support `--`, forcing an absolute path guarantees the string begins with `/` instead of `-`. For utilities that do support it (like `xattr`), placing `--` before variable paths is the safest approach.
**Prevention:** Always use `--` to delimit arguments from positional parameters in shell executions, or force dynamic strings representing paths to an absolute format (`str(path.absolute())`).
