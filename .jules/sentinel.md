...
## 2026-04-18 - [MEDIUM] Fix Command-Line Argument Injection in subprocess calls
**Vulnerability:** The application used `subprocess.run` to call system utilities (`xattr`, `diskutil`) with user-controlled or variable file paths without explicitly separating options from positional arguments. A malicious filename starting with a hyphen (`-`) could be interpreted as a flag by the utility, potentially leading to command-line argument injection or unexpected behavior.
**Learning:** Utilities that parse command-line arguments can mistake paths for options if they start with `-`. When calling system commands via `subprocess`, paths must always be sanitized or explicitly demarcated.
**Prevention:** For utilities that support it (like `xattr`), use the POSIX `--` delimiter before the path argument to indicate the end of options. For utilities that do not support `--` (like `diskutil`), convert the path to an absolute path (`str(path.absolute())`) so it always starts with `/` and is never interpreted as a flag.
