...
**Vulnerability:** Subprocess calls (`subprocess.run`) executing system commands like `xattr` and `diskutil` passed user-controlled file paths as arguments without protection against paths that start with a hyphen (`-`). If a malicious file was named something like `-someflag`, it would be interpreted as an option/flag by the system command rather than a file path, potentially leading to command injection, arbitrary option execution, or unexpected behavior.
**Learning:** Even when avoiding shell execution (`shell=False`) in subprocess calls, the arguments passed to commands must be protected to ensure that positional arguments (like file paths) are not mistaken for options. This is a common form of injection known as Argument Injection or Option Injection (CWE-88).
**Prevention:** For commands that support it (like most POSIX commands), always insert the `--` delimiter before the positional arguments (e.g., `["xattr", "-p", "name", "--", str(file_path)]`) to signal the end of command options. For utilities that do not support `--` (like macOS `diskutil`), transform the argument so it cannot start with a hyphen. Converting relative paths to absolute paths (`str(path.absolute())`) guarantees the string will begin with a slash (`/`), rendering it safe from option injection.
