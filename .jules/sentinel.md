...
## 2026-05-10 - [CRITICAL] Fix Command-Line Argument/Option Injection in Subprocess Calls
**Vulnerability:** The `criteria_matcher.py` and `local_drive_identity.py` components passed unescaped variable paths into `subprocess.run` (e.g., `["xattr", "-p", "com.apple.lastuseddate#PS", str(file_path)]`). A maliciously named file starting with `-` could be interpreted as a command-line option rather than a path, allowing attackers to inject arbitrary arguments and modify utility execution logic (CWE-88).
**Learning:** Utilities that accept variable inputs as arguments can interpret them as flags if the inputs start with a hyphen (`-`). This bypasses the protections of passing lists to `subprocess.run` instead of single strings (shell injection prevention).
**Prevention:** Always use the standard POSIX `--` end-of-options delimiter before passing variable paths/arguments to commands that support it (e.g., `["xattr", ..., "--", str(file_path)]`). For utilities that do not support `--` (like `diskutil`), convert the path to an absolute path first (`path.absolute()`), which guarantees it starts with `/` rather than `-`.
