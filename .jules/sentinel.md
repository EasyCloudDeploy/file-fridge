...
## 2026-02-12 - [HIGH] Fix Stored XSS in Tag Color
**Vulnerability:** The `color` field in `TagCreate` and `TagUpdate` schemas lacked validation, allowing arbitrary strings (including XSS payloads) to be stored in the database. When rendered in the frontend, this could lead to Stored XSS.
**Learning:** Optional fields in Pydantic schemas, even if they seem benign (like a color string), must be strictly validated if they are rendered dynamically in the UI. XSS payloads can be injected anywhere.
**Prevention:** Use a centralized helper function with strict regex validation (like `^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})$`) for format-specific fields, and apply it via Pydantic validators.
