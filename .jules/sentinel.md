...
## 2026-02-22 - [CRITICAL] Fix Remote Transfer Header Integrity Vulnerability
**Vulnerability:** The remote file transfer protocol signed the request method, URL, and body, but failed to sign critical metadata headers (e.g., `X-Relative-Path`, `X-Chunk-Index`). This allowed a Man-in-the-Middle attacker (or compromised instance) to modify these headers in transit while keeping a valid signature, potentially overwriting arbitrary files within allowed paths.
**Learning:** Digital signatures must cover ALL data that influences the logic or security decisions of the receiver. Relying on unsigned headers for control flow (like file paths or job IDs) breaks the integrity guarantee of the signature.
**Prevention:** Use a `SIGNED_LOGIC_HEADERS` allowlist to explicitly include sensitive headers in the canonical signing string. Ensure that both the client (signer) and server (verifier) agree on exactly which headers are protected.
