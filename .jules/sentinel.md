...
## 2026-04-26 - [CRITICAL] Fix Command-Line Argument Injection
**Vulnerability:** Calls to `subprocess.run` with utility functions like `xattr` or `diskutil` directly injected a user-controllable (or dynamically generated) string (`str(file_path)`). If the file path started with a hyphen (`-`), the utility would interpret it as a flag or option rather than a positional file argument, potentially causing unintended side effects, command failures, or in specific tools, execution of arbitrary actions.
**Learning:** File paths can contain any character, including hyphens. When passing untrusted or dynamic path names to external shell utilities via `subprocess`, they must be definitively marked as positional arguments to avoid option parsing.
**Prevention:** Use the POSIX `--` delimiter before positional arguments (e.g., `["xattr", "-p", "name", "--", str(file_path)]`). For non-POSIX utilities (like macOS `diskutil`), convert the path to an absolute path (`str(path.absolute())`) so it is guaranteed to begin with `/` and not `-`.
