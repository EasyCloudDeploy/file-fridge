...
## 2026-05-04 - [HIGH] Fix Command-Line Argument Injection in subprocess calls
**Vulnerability:** Calls to `subprocess.run` (e.g., executing `diskutil` or `xattr`) accepted dynamic file paths without proper delimiters. If a user-controlled file path began with a hyphen (e.g., `-foo.txt`), the command-line utility could misinterpret the file path as an option/flag, potentially leading to unauthorized behavior, errors, or security bypasses (CWE-88).
**Learning:** Command-line interfaces often parse arguments strictly based on a hyphen prefix. When passing variable data to a command's argument list, you must use standard POSIX conventions (like the `--` separator) to signify the end of options, or ensure the argument cannot look like a flag (e.g., by making the path absolute so it starts with `/`).
**Prevention:** Always use the `--` separator when invoking subprocess commands with dynamic arguments (e.g., `["xattr", "-p", "name", "--", str(path)]`). If the command does not support `--` (like macOS `diskutil`), convert the path to an absolute path (`str(path.absolute())`) before execution.
