...
## 2026-04-20 - [CRITICAL] Prevent Command-Line Argument/Option Injection (CWE-88) in subprocess calls
**Vulnerability:** The application invoked the `xattr` command line utility via `subprocess.run` with dynamically provided file paths as arguments. Because the file paths were appended without a `--` delimiter, an attacker could supply a path starting with `-` (e.g., `-rf`), which the target command would incorrectly parse as a command-line option rather than a file path, leading to option injection. Note that some macOS utilities like `diskutil` do NOT support `--` and will fail if provided.
**Learning:** Any dynamically-provided argument to a command line tool might inadvertently be treated as an option flag if it begins with a dash. However, not all tools support the standard `--` POSIX delimiter.
**Prevention:** Always insert the double-dash `--` argument before any variable arguments in `subprocess.run` calls to explicitly signal the end of command options and the beginning of positional arguments, *after* verifying the target utility supports it.
