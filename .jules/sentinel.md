...
## 2026-05-15 - [HIGH] Fix Command-Line Argument Injection in subprocess calls
**Vulnerability:** Calls to `subprocess.run` with utilities like `diskutil` and `xattr` constructed command lists where the user-controlled file path was directly appended as the last argument. If a file path started with a dash (e.g., `-rf`), these utilities could interpret the file path as an option/flag rather than a positional argument, leading to Command-Line Argument Injection (CWE-88).
**Learning:** Even when avoiding `shell=True` to prevent arbitrary command execution, `subprocess.run` is still vulnerable to Argument Injection if the invoked program parses arguments dynamically and doesn't explicitly know where options end and positional arguments begin.
**Prevention:** For POSIX-compliant utilities (like `xattr`, `rm`, `ls`), always insert the `--` delimiter before any variable or user-provided arguments to explicitly signal the end of options (e.g., `["xattr", "-p", "name", "--", str(path)]`). For utilities that do not support `--` (like macOS `diskutil`), convert the path to an absolute path (`str(path.absolute())`) so that it is guaranteed to start with `/` rather than `-`.
