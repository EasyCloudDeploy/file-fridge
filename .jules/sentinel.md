...
## 2026-05-09 - [HIGH] Fix Command-Line Argument/Option Injection
**Vulnerability:** Calls to `subprocess.run` involving system utilities (`xattr`, `diskutil`) used user-controlled file paths as arguments without explicit option delimiters (`--`) or absolute path resolution. This could allow an attacker to create files with names starting with `-` (e.g., `-h` or malicious flags) which the utilities would interpret as options, potentially leading to unintended behavior or arbitrary file reads/modifications depending on the utility.
**Learning:** Utilities spawned via `subprocess.run` (or similar) will interpret any argument starting with `-` as a flag unless strictly delimited. This is a common injection pattern even if `shell=False` is used, because the target binary itself handles argument parsing.
**Prevention:** For POSIX-compliant utilities (like `xattr`), always place `--` before variable path arguments to signal the end of options. For utilities that do not support `--` (like `diskutil`), convert the path to an absolute path (`str(path.absolute())`) to ensure the argument string begins with a `/` rather than a `-`.
