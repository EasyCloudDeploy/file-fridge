...
## 2026-04-22 - [HIGH] Fix Command-Line Option Injection (CWE-88) in xattr and diskutil
**Vulnerability:** The application invoked `subprocess.run` with commands `xattr` and `diskutil` passing user-controlled or variable file paths without the POSIX `--` end-of-options delimiter. If a file path started with a hyphen (e.g., `-rf`), the command line utility could interpret it as an option instead of a positional argument, leading to unexpected behavior or potential command injection depending on the utility.
**Learning:** Utilities that parse command line options generally stop parsing options when they encounter the `--` delimiter. Always use `--` when passing variable strings (like file paths) to `subprocess.run` commands to prevent option injection.
**Prevention:** Always include `--` in `subprocess.run` arguments before any variable file paths, especially when those paths are influenced by user input or the filesystem. Check `subprocess` calls for missing `--` delimiters during code review.
