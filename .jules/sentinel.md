...
## 2024-04-24 - [CRITICAL] Fix Path Traversal Bypass in `check_path_permission`
**Vulnerability:** The `check_path_permission` logic used `Path.absolute()` along with `.relative_to()` to restrict directory access. However, `relative_to` only performs textual prefix matching and `absolute()` does not resolve `..` elements. An attacker could supply `/allowed_path/../../etc` which would textually appear relative to `/allowed_path` but actually resolve to system directories, enabling path traversal bypassing the restriction.
**Learning:** In Python's `pathlib`, `absolute()` does not eliminate `..` traversal components. Only `.resolve()` interrogates the file system (or performs strict structural normalization) to eliminate `..` elements. Security boundaries relying on prefix matching must fully resolve paths first.
**Prevention:** Always use `.resolve()` on untrusted paths before utilizing `.relative_to()` for permission checks. Additionally, use `try ... except OSError` and fail securely (deny access) when path resolution fails.
