...
## 2026-04-18 - [HIGH] Fix Command-Line Option Injection (CWE-88) in Subprocess Calls
**Vulnerability:** Calls to `subprocess.run` (e.g., executing `xattr` or `diskutil`) used user-controlled file paths as arguments without the standard `--` argument delimiter or path absolute conversion. This allowed Command-Line Argument/Option Injection, where a file specifically named to look like a flag (e.g., `-rf`) would be executed as an option instead of a file path, potentially causing denial of service or arbitrary behavior.
**Learning:** Utilities that process file paths provided via `subprocess` may misinterpret a filename starting with a `-` as a command option unless mitigated. Some programs support `--` to denote the end of command options, while others require the path to begin with an absolute directory structure (e.g., `/`).
**Prevention:** Always use the `--` POSIX argument delimiter for utilities that support it (e.g., `xattr`, `mdls`) before variable file paths. For commands that don't (like macOS `diskutil`), convert the path to an absolute path (`str(path.absolute())`) so that it strictly starts with `/` and is never misinterpreted as an option flag.
