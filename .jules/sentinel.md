...
## 2026-05-18 - [CRITICAL] Fix Command-Line Argument/Option Injection in subprocess.run
**Vulnerability:** The application passed user-controllable or variable file paths directly to `subprocess.run` calls for utilities like `xattr` and `diskutil` without properly ensuring they wouldn't be interpreted as command-line options. If a path began with `-`, the utility could interpret it as a flag rather than a target file, leading to Command-Line Argument Injection (CWE-88).
**Learning:** Simply passing arguments as a list to `subprocess.run` (instead of using `shell=True`) prevents arbitrary shell command execution (Command Injection), but it does NOT prevent Option Injection if an attacker can control a string that is passed as a positional argument where the program expects either an option or a filename.
**Prevention:** For utilities that support the POSIX `--` end-of-options delimiter (like `xattr` or `mdls`), insert `--` before the path argument. For utilities that do not support `--` (like `diskutil`), ensure the path always starts with `/` by converting it to an absolute path (`path.absolute()`), so it can never be parsed as a flag.
