...
## 2026-05-16 - [HIGH] Fix Command-Line Argument/Option Injection
**Vulnerability:** The application passed user-controlled file paths directly to external commands via `subprocess.run` (e.g., `["xattr", "-p", "com.apple.lastuseddate#PS", str(file_path)]` and `["diskutil", "info", "-plist", str(path)]`). If a user created a file with a name starting with `-` (like `-h` or `--delete`), the external command would interpret the path as a command-line flag rather than a target file, leading to option injection. This could potentially allow denial of service, unexpected behavior, or even arbitrary code execution depending on the underlying tool.
**Learning:** Even when avoiding `shell=True`, passing raw paths to external commands is unsafe if the tool processes arguments positionally and might interpret leading dashes as flags. POSIX tools usually support the `--` delimiter to indicate the end of options, while others may require absolute paths to guarantee they start with a forward slash `/` instead of `-`.
**Prevention:** Always insert the `--` delimiter before user-controlled file paths in `subprocess` lists when calling standard POSIX utilities (e.g., `["cmd", "--", path]`). For utilities that do not support `--` (like macOS `diskutil`), convert the path to an absolute path first (`path.absolute()`) so it invariably begins with `/`, preventing it from being parsed as a flag.
