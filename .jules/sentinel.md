...
**Vulnerability:** Calls to `subprocess.run` (e.g. `xattr` or `diskutil` commands) included variable paths without a `--` delimiter. If an attacker controlled a file path and named it starting with a dash (e.g., `-h` or `-p`), it could be parsed as an unexpected option/flag by the executed command, potentially resulting in undefined behavior or Command-Line Argument/Option Injection (CWE-88).
**Learning:** Utilities spawned via `subprocess.run` without `shell=True` can still be vulnerable to argument injection if user-provided input is parsed as options rather than positional arguments.
**Prevention:** Generally, insert `--` before the file path argument (e.g., `['xattr', '-p', 'name', '--', str(file_path)]`) to signify the end of options. For utilities that do not support the POSIX `--` delimiter (e.g., macOS `diskutil`), convert the path to an absolute path (e.g., `str(path.absolute())`) so it strictly starts with `/`, preventing it from being parsed as a flag.
